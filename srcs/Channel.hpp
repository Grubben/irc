...
public:
	...
	void 					setPassword(bool sign, std::string pass) { if (sign == false && pass == _password){_password = ""; _hasPassword = false; } _password = pass; _hasPassword = true; }
	void 					setMaxUsers(bool sign, int max) { if (max == 0 || sign == false){ _maxUsers = 0; _userLimit = false; } _maxUsers = max; _userLimit = true; }
